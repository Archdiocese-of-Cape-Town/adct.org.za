...
Under **Events → All Events**, use the Parish, Next date, Recurring, Featured and event status filters together to find a post; the list also shows these columns. Next date is the next stored occurrence from now, not the series' original start. Open an event to change its content, event types, parish, venue, schedule, recurrence, featured checkbox, status and private contact details. The featured checkbox is an explicit editor choice, including when left unchecked: later parser suggestions cannot replace it. The offline parser only suggests Featured for once-off conference, pilgrimage, jubilee or ordination notices; confirmation and approval remain required before any new event is published.
