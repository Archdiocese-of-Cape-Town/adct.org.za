...
Insert the **Upcoming events** block in the block editor, or put `[adct_events]` in a page. The optional shortcode attribute `period="week"` or `period="month"` selects a preset; the default lists the next year. Visitors can choose This week, This month or a custom **From / Through** date range and submit the form without JavaScript. Dates are ISO `YYYY-MM-DD` in the URL; ranges must intersect today through one year from today, and requests are limited to 20 events per page and 100 pages (2,000 entries). At the cap, a notice asks visitors to narrow their dates if more events remain. Links to next/previous pages preserve the selected filter. The listing only displays published event posts and public title/date/location/type/recurrence/featured/status fields; it never includes contact details or source emails. Place the block or shortcode on a page with an appropriate permalink and link to that page from the site's navigation when ready; the existing Google Calendar embed remains unchanged until separately replaced.
...
Open **Parish Intake → Health** (requires report access). The background jobs table shows the last run, whether cron or a person started it, and the last success. Mailboxes and sources show their last check, last success, last item and consecutive failures; open **Sources** for technical error details. The waiting work section shows saved inbound mail waiting to parse, messages currently extracting, and outbound email waiting or permanently failed. Mail waiting on the IMAP server has not been downloaded yet and cannot be counted here.

If a job has not run for **more than 2 hours 15 minutes**, check that WP-Cron is enabled, the xneelo two-hour HTTP backstop is still configured and the optional pinger can reach `wp-cron.php`. See [Keep scheduled jobs running](#keep-scheduled-jobs-running). If a mailbox fails repeatedly, inspect **Parish Intake → Mailboxes**, verify its password/connection and check its source status; an Unreliable source needs an operator to reactivate it after the connection is fixed. If mail accumulates, check the SMTP plugin and the hourly allowance below. If a deanery has no active approver, use **Parish Intake → Deaneries** to assign one; meanwhile archdiocese reviewers can handle its items.

**Check now** requires settings access and runs mailbox polling, parsing and the outbound queue in three batches of at most 25 seconds each, using the normal locks and checkpoints. It can send *queued* mail, so enable outbound Test mode first on non-production sites. The page never runs a job just by being viewed.

After **3 consecutive failures** of a source or job, or a stalled heartbeat, the plugin queues one alert per incident to the configured administrator email. A recurrence after recovery is limited to **one alert per 24 hours per source/job**. Alerts share the same test-mode rules, queue, priorities and 100-recipient/hour default cap as other plugin mail; they cannot be delivered while the mail sender or cron is stopped, but the dashboard remains visible. Set `ADCT_PI_HEALTH_ALERT_EMAILS` to an array of administrator addresses in `wp-config.php` to override the site's administrator email; never put addresses in the repository. Alerts do not contain source identifiers, error details or message content.

The SMTP warning detects common active SMTP plugins (FluentSMTP, WP Mail SMTP, Post SMTP); activation alone does not prove SMTP is configured. Verify authenticated SMTP and DKIM separately. A warning with no recognised plugin is a prompt to check the site's actual delivery path, not proof that mail has failed.
...
The screen shows verdicts and their configured trust status, not raw headers or message contents. A reported DMARC fail is visibly flagged for review; the Core `InboundMailPolicy::canApplyInstantChange()` also blocks instant changes for that message from a verified contact. The approval route will consume this policy in #71. Inbox processing leaves candidates as drafts for review and does not send confirmation or approval email.
