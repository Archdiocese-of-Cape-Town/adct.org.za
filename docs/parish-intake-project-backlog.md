...
| [E4.7](https://github.com/Archdiocese-of-Cape-Town/adct.org.za/issues/76) | Outbound email queue with hourly cap and priorities | P0 | E0.6, E0.7 | All plugin mail uses `MailerInterface` → queue → `wp_mail`; rolling cap defaults to 100 successful recipients/hour and is configurable from `wp-config.php`; atomic claims reserve capacity; login/confirmation first; exponential retries stop after 5 attempts; per-recipient `group_key` idempotency; recipient-policy seam for #48; status API supplies pending count, oldest age and terminal failure count for #64. |
