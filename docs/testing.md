...
| 3. WordPress integration | Activation from the built release zip without PHP errors/notices; schema v2 sets version 2, creates all 15 `adct_pi_*` tables, upgrades venue metadata and preserves the legacy table; directory imports seed provisional defaults and linked outstation venues idempotently; venue creation, default selection, location lookup and the parish Venues tab work. | GitHub Actions using `wp-env` (Docker) and WP-CLI | Every PR |
