...
| 3. WordPress integration | Activation from the built release zip without PHP errors/notices; schema v2 sets version 2, creates all 15 `adct_pi_*` tables, upgrades venue metadata and preserves the legacy table; directory imports seed provisional defaults and linked outstation venues idempotently; venue creation, default selection, location lookup and the parish Venues tab work; the Parish Intake menu and Manual parser page register and render, and the parser renders every bulletin candidate while storing the first in the legacy table. | GitHub Actions using `wp-env` (Docker) and WP-CLI | Every PR |
