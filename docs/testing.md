...
| 3. WordPress integration | Activation from the built release zip without PHP errors/notices; schema v1 sets version 1, creates all 15 `adct_pi_*` tables and preserves the legacy table; the Parish Intake menu and Manual parser page register and render for an administrator; on submission, the parser renders every bulletin candidate while storing the first candidate in the legacy table. | GitHub Actions using `wp-env` (Docker) and WP-CLI | Every PR |
...
- Each fixture is a pair: `tests/fixtures/emails/<name>.eml` and `tests/fixtures/emails/<name>.expected.json`. Existing top-level parse fields continue to compare against the first/primary `ParseResult`. Bulletin fixtures may additionally provide `candidate_count` and a `candidates` list; each candidate can assert `block_index`, `fields` (including title/date/time), and other `ParseResult::toArray()` values. A missing expected key is not checked; list lengths are checked. `known_failures` maps an output path (or parent path) to an existing issue, such as `{"fields.description":"#74"}`. Mismatches covered by known issues are reported as incomplete; any untracked mismatch fails with a per-field diff. Keep the fixture and update the expectation only when the relevant issue is fixed.
...
- The runtime MIME parser is pure PHP and does not require `ext-imap` or `ext-dom`. Added invented fixtures cover Outlook HTML, Gmail and Apple Mail alternatives, mobile signatures, forwarded and quoted replies, Windows-1252, and a Mailchimp-style newsletter; never include real parish samples.
- To add a fixture, add those two files, then run `composer test` and `composer fixture-score`. The PHPUnit provider discovers fixture pairs automatically. Bulletin examples should assert all candidates with `candidate_count` and `candidates`; single-event fixtures may keep their existing top-level fields unchanged.
...
- **WordPress Playground PR previews** (`playground.wordpress.net`): free, no account, runs WordPress in the browser.
  ...
  - The blueprint `.github/playground/blueprint.json` installs and activates the release zip, logs in as `admin`, and opens the Manual parser page. **TODO (follow-up now that #22 has landed):** add anonymised parish and event records through a fixture/seed mechanism for schema v1; the current blueprint intentionally does not load application data until that mechanism exists.
