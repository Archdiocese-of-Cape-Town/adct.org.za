...
| 3. WordPress integration | Activation from the built release zip; schema v7 creates all 17 `adct_pi_*` tables and the mail-queue composite unique index; simulated v3-to-v7 and v4-to-v5 duplicate-preservation migrations, v5-to-v6 rate-limit migration and v6-to-v7 confirmation metadata migration; action-token GET/POST, nonce and renewal-limit checks; event type/taxonomy registration, occurrence rebuilds and rollback, REST metadata privacy and role authorization, public listing/block pagination and cache behavior, and uninstall capability cleanup; mailbox settings, polling, duplicate/skipped-message reporting, safe Inbox status/error rendering and same-row reprocessing without candidate, attachment, checkpoint, mail or privacy regressions; confirmation previews for multiple candidates, safe recipient selection and suppression, cross-recipient queue recovery, bound action tokens, thread headers and `example.test`-guarded queue delivery; directory, source, venue, deanery, parish-contact and Manual parser integration. | GitHub Actions using `wp-env` (Docker) and WP-CLI | Every PR |
...
Issue #59 covers the queue's disjoint primary categories alongside the overlapping all-items approval view, scoped tab counts, menu badge, sender/parish/title search, read-only preview and submitter view, conditional first-wins bulk decisions with one audit entry per winner, unsafe-match exclusion, parish reassignment without sender verification, and dean/reviewer capability and nonce checks against the installed release ZIP. Instant-change and Revert integration is deferred to #71; the queue never represents ordinary approved updates as instant changes.
