...
- Unit tests cover the time and item budgets, a checkpoint saved after every item, resumption from that checkpoint, due checks, overlapping-run prevention, expired-lock recovery, stale-token release protection, and per-run lifecycle reset for stateful jobs.
