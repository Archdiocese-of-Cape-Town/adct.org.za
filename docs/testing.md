...
|---|---|---|---|
| 1. Unit | Domain core: parsing stages and directory lookup, date/recurrence, matching, trust rules, tokens, MIME parsing, IMAP protocol, and versioned snapshot cache invalidation with fakes. Includes a token-based check that `src/Core/` has no WordPress dependencies. | GitHub Actions + locally | Every push / PR |
