...
7. **Parishes without a deanery** (movements, archdiocesan offices, groups) go to archdiocese reviewers only.
   *Clarification (2026-09-24):* the same applies to a parish whose deanery has **no active approver** yet (e.g. the dean has no user account or email set up). Approval never waits on a dean who isn't set up. The system works with archdiocese reviewers alone, and deans can be added gradually. The admin dashboard lists deaneries without an approver.
