...
3. **InstaWP / TasteWP** are used for real mail tests. Install the CI-built zip by URL, connect a **test mailbox**, and enable **Parish Intake → Outbound email → Test mode** before testing. Test mode is off by default and limits only Parish Intake queue mail to exact allow-listed email addresses and domains; other recipients are logged as suppressed. It does not change other plugins' mail. Sites expire, and only throwaway test credentials are used there. InstaWP's own GitHub deployment is optional; running Composer on its side is a paid feature, so the zip is simpler.
