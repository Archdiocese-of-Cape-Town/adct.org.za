...
| `queue_confirmation_previews` | due every 10 min | Select the oldest parsed inbound message with draft candidates and no confirmation outcome; compose one HTML/text preview for all its draft candidates, choose a safe sender or verified Reply-To, and enqueue it with bound action tokens and validated thread headers. |
