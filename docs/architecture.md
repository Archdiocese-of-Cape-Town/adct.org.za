...
- `WordPress\Ai\OpenAiCompatibleProvider` implements the core AI port and receives `HttpClientInterface`; only `WordPress\Http\WordPressHttpClient` calls `wp_remote_post`. An atomic daily reservation uses the existing hashed rate-limit table; HTTP 429 and 5xx set a cooldown. Cron-driven inbox processing alone enables AI: the Manual parser and admin-triggered synchronous reprocessing use deterministic parsing.
...
- Public views: the server-rendered `[adct_events]` shortcode and `adct/events` block read bounded pages from the indexed occurrences table, join only published posts, and render day-grouped cards with local dates. Preset week/month and validated custom ranges share ordinary GET links, so JavaScript is not required. Short-lived transients use fixed keys for at most three presets and 100 pages; their stored generation and time window must match before reuse. Arbitrary date ranges bypass transient writes to avoid unbounded options growth. An event write, status change, term assignment or occurrence replacement rotates the generation, and cached rows are rechecked against the published post status before display. The public `?adct_ics=1` calendar endpoint selects up to 500 distinct published events with occurrences in the rolling year; optional `parish` (ID) and `type` (term slug) filters avoid unbounded transient keys, while the all-events feed uses one fixed 60-second cache key. Its stable UIDs and ETags share listing invalidation. Single event templates remain separate work.
...
| `process_queue` | due every 10 min | Planned: route reviewed candidates through confirmation and approval; no confirmation or approval mail is sent by inbound parsing. |
