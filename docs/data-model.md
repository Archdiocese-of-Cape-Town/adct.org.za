...
For **new** events, self-approval additionally requires the bound confirmation recipient to equal the inbound sender, an active WordPress user at that address with reviewer capability or an active deanery assignment for the candidate's parish, no blocked contact or reported DMARC failure, and no pending-match review flag. Sender verification by itself is never sufficient. Ambiguous matches and changes/cancellations remain in approval rather than using the instant-change path (#71). Publication after the committed self decision uses the existing publisher; an interrupted publish can be retried with the same nonce-protected POST, which repairs only that actor's pending self-approval and never records a second decision.
