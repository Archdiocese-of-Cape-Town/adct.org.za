...
| matched_candidate_id, match_event_id, match_kind, match_review_required | `new`, `update`, `duplicate`, `cancellation`, `postponement` |
...
### `adct_pi_mail_queue`
recipient, subject, body_html, body_text, priority (1 = login/confirmation, 2 = approver/change notice, 3 = reminder/digest), group_key, `thread_headers`, `payload_fingerprint`, status (`queued`, `sending`, `sent`, `failed`, `suppressed`), attempts, next_attempt_at, sent_at, error. Each row is one validated recipient; outbound messages have no BCC or attachments. Confirmation previews store validated `In-Reply-To` / `References` values as JSON and a SHA-256 fingerprint of the composed input; both are nullable for other mail. The sender respects an hourly cap (default 100, configurable from `wp-config.php`, bounded to 1–500) because the host allows 500 recipients per hour for the whole account ([ADR 0011](decisions/0011-outbound-email-queue-with-hourly-cap.md)). Test mode is an option-backed, off-by-default queue policy; it is checked at enqueue and send time, and blocked rows retain the `suppressed` status and a safe error code. The settings-only Outbound email screen shows at most 20 escaped previews, each limited to 2,000 body characters; this is not exposed through REST. Sent rows are pruned after 30 days, and the status API exposes pending count, oldest pending age, successful sends in the preceding hour and terminal failure count without exposing message bodies.
...
The confirmation-preview job uses `confirmation:<inbound-message-id>` as its group key and records `queued`, `sent`, `suppressed` or `failed` plus a safe reason on the inbound message. Before applying current sender-trust, automated-mail or safe-recipient suppression to a retry, it checks up to two queue rows with that group key across all recipients. A single existing row is authoritative: the job verifies the current safe recipient route and payload fingerprint, then reuses its persisted status without minting tokens. This prevents a marker-write retry from changing a queued or sent outcome to suppressed after trust or automation flags change. For an eligible new preview, it checks the fixed group key again before issuing tokens. An unresolved or changed recipient, changed payload, or multiple matching rows is a terminal `queue_conflict`; the job never silently enqueues a second confirmation to another address. A transient job failure before the inbound marker is written is retried on a later run; an existing row with a matching fingerprint is reused. Queue-level test-mode suppression and terminal delivery failure are recorded and are not automatically requeued. Permanently missing, corrupt or oversized raw headers are recorded as `raw_message_unavailable` and the bounded job continues with the next inbound message; transient storage and database failures still retry. `queued` means the queue accepted the email; `sent` means WordPress accepted it for delivery, not that it reached the recipient's inbox.

The confirmation marker does not change candidate state. Candidate action handling is a separate workflow step; enqueuing a preview alone cannot confirm, deny or publish candidates.

The E4.3 confirmation handler enables **Confirm**, **Deny** and **Confirm all** (Edit remains inactive). Only a nonce-checked POST using the bound, unexpired confirmation token can act; the stored queue item must have the same recipient and a deliverable `queued`/`sending`/`sent` status, and the inbound confirmation marker must be `queued` or `sent`. Suppressed or failed previews cannot act. A transaction locks all selected candidates, consumes the token, changes each draft state and inserts one audit record per candidate; a competing or replayed action cannot decide the same candidate twice. Confirmed candidates enter `awaiting_approval` for the parish's deanery approvers (when assigned and active) and archdiocese reviewers; queue screens and notifications are separate issues. Denial stores the optional reason in `decision_note` and the audit details. `unknown_sender` and reported `dmarc_fail` flags are appended to candidate review notes and audit details for approvers. Confirm-all skips candidates already decided individually, while a per-event action cannot act on a previously decided candidate.

### `adct_pi_approval_notices` (v8)
One row per candidate/recipient with a unique `(candidate_id, recipient)` key, a grouped mail-queue `group_key`, notification mode and `queued_at`. The bounded approver job writes assignments before composing each email and recovers unfinished groups first; a queue row under the same recipient/key is reused after a crash rather than minting new action links. Daily digests use a per-recipient local-date key and newly confirmed candidates after that day's digest wait until the next day. The job never sends attachments. Approve/reject actions verify that the notice has a deliverable queue row and that the approver is still active and authorized. Decisions write `decided_by` / `decided_at` and an audit entry atomically, then approval calls the idempotent publisher; failed publication is recoverable with the winning token. Submitter live/rejection mail uses fixed per-candidate queue keys.

For **new** events, self-approval additionally requires the bound confirmation recipient to equal the inbound sender, an active WordPress user at that address with reviewer capability or an active deanery assignment for the candidate's parish, no blocked contact or reported DMARC failure, and no pending-match review flag. Sender verification by itself is never sufficient. Ambiguous matches and all matched changes/cancellations/postponements remain in approval rather than using the instant-change path (#71). Publication after the committed self decision uses the existing publisher; an interrupted publish can be retried with the same nonce-protected POST, which repairs only that actor's pending self-approval and never records a second decision.
