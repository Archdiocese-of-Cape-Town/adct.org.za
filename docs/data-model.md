...
Candidate matching locks the inbound source row and compares at most 200 recent candidates from the **same source and parish**, so simultaneous bulletins cannot both create reviewable copies. Matching uses normalized titles (60% of the score) and matching dates or overlapping occurrences under identical normalized RRULEs (40%). Recurrence overlap is checked by expanding both rules for a bounded 12-month window beginning at the later series anchor; a rule match without a shared occurrence is not enough. A changed date only qualifies with an explicit change/cancellation/postponement phrase, title similarity of at least 0.95, and a 60-day bound; this evidence scores 0.8 in the date component. Matches need a score of at least 0.88 and a lead of 0.08 over the runner-up; otherwise the candidate remains `draft` with `fields.match_review_required` and a review note. Published targets must still be the current published `adct_event` with matching parish, source candidate, title, description, start and recurrence metadata. Unchanged notices are retained as audited `status = duplicate`, `match_kind = duplicate` candidates rather than removed; repeated unpublished candidates link via `fields.matched_candidate_id`. Changes to unpublished candidates retain their change kind and pending-candidate link but remain drafts with `fields.match_review_required`; they are not publishable updates without a published event ID. Explicit changes to published events remain drafts with `match_event_id` and must pass the normal confirmation and approval workflow. A notice saying it was postponed/rescheduled/moved **to** a replacement schedule extracts that schedule rather than the preceding original date/time. If no usable replacement date or time follows, `fields.replacement_schedule_unresolved` blocks automatic event matching and flags the candidate for manual review. Original inbound raw mail remains governed by its existing retention policy.
...
The confirmation marker does not change candidate state. Candidate action handling is a separate workflow step; until those handlers are registered, preview email links are explicitly inactive and cannot confirm, deny, edit or publish candidates.
