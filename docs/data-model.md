...
| confirmation_status, confirmation_reason | nullable outcome (`queued`, `sent`, `suppressed`, `failed`) and safe reason code (`no_candidates`, `blocked_sender`, `automated_or_list`, `no_safe_recipient`, `test_mode`, `delivery_failed`); this marker is separate from the inbound processing status |
...
The confirmation-preview job uses `confirmation:<inbound-message-id>` as its group key and records `queued`, `sent`, `suppressed` or `failed` plus a safe reason on the inbound message. A transient job failure before the inbound marker is written is retried on a later run; an existing row with a matching fingerprint is reused. Queue-level test-mode suppression and terminal delivery failure are recorded and are not automatically requeued. `queued` means the queue accepted the email; `sent` means WordPress accepted it for delivery, not that it reached the recipient's inbox.
